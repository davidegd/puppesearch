...
const transformData = data => {
  const prices = data.map(e => {
    return {
      ...e,
      price: Number(Number(e.price.substr(1, e.price.length)) + (Number(e.price.substr(1, e.price.length)) + 20) * 0.08 + Number(e.price.substr(1, e.price.length)) * 0.4).toFixed(2)
    };
  });
  const used = prices.filter(({
    title
  }) => title.includes("Used")).sort((a, b) => Number(a.price) - Number(b.price));
  const news = prices.filter(({
    title
  }) => !title.includes("Used"));
  ...
  try {
    ...
    const products = await page.evaluate(() => {
      ...
      for (let index = 0; index < elements.length; index++) {
        links.push({
          title: elements[index].querySelector(".product-card-title a").innerText,
          price: elements[index].querySelector(".product-card-price .sale-price").innerText,
          img: elements[index].querySelector(".product-card-image a img").src
        });
      }
      return links;
    });
